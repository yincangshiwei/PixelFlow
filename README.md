...
| 模块 | 说明 |
|------|------|
| 文件操作栏 | 添加文件、添加文件夹、移除选中、清空；支持拖放图片/文件夹；HTML/DOCX/PDF 自动提取内嵌图片；Ctrl+V 粘贴文件路径、文件夹、截图及 HTML 图片 |
...
| 区域 | 说明 |
|------|------|
| **输出设置** | 四种输出路径模式 + 自动创建文件夹 + **保留目录结构** + **覆盖同名文件** +（透明图开启 AI 抠图时）**保留抠图结果** + **处理范围（全部文件 / 仅选中）** |
| **底部操作栏** | 进度条 + 开始处理 / 取消按钮 |
...
| **覆盖同名文件**（默认关闭） | 勾选后若输出目录已存在同名文件则直接覆盖；未勾选时自动在文件名后追加 `_1`、`_2`… 避免覆盖 |

原图路径模式（覆盖/副本）下上述三项自动隐藏（「原图路径 (覆盖原图)」本身即为覆盖写回）。

透明图处理且勾选 **AI 抠图** 时，输出设置额外显示：

| 选项 | 说明 |
|------|------|
| **保留抠图结果**（默认关闭） | 额外保存一份 AI 抠图后的透明 PNG；若同时开启裁剪透明边缘，保留的是裁剪后的透明主体。文件名在最终主输出名后追加 `_matted`（例如 `photo.png` → `photo_matted.png`；若因重名变为 `photo_1.png`，则为 `photo_1_matted.png`） |
...
| 步骤 | 参数 | 说明 |
|------|------|------|
| **AI 抠图** | 模型、边缘精炼 | 使用本地 AI 去除背景；可选 **BEN2**（[ModelScope](https://www.modelscope.cn/models/PramaLLC/BEN2)）、**RMBG 2.0**（[ModelScope](https://www.modelscope.cn/models/briaai/RMBG-2.0)）。每模型独立 uv 环境，主程序通过子进程调用；需先在配置页安装 uv、创建环境并下载权重。边缘精炼仅 BEN2 支持。开启后，输出设置可勾选 **保留抠图结果**（见上文输出路径选项） |
| **裁剪透明边缘** | Alpha 阈值 (0-254) | 去除四周多余透明空间。阈值为 0 表示仅裁剪完全透明像素；调高可忽略半透明边缘 |
| **画布与主体布局** | 画布宽高、背景、四边预留、主体大小、主体位置、细节恢复 | 四边预留按画布百分比计算；主体在扣除预留后的可用区内等比缩放，并可按九宫格位置对齐 |

例如 `1000×1000` 画布设置顶部预留 `10%`、底部预留 `20%`，可用区为 `1000×700`；主体大小 `80%` 时会等比完整放入 `800×560` 的安全框，再按左上、居中、右下等位置在可用区内对齐。**无论源图偏大或偏小，都按同一规则缩放**，保证批量产品图标准化呈现；源本身分辨率不足时放大无法凭空变清。

**典型使用场景：** 电商产品图批量处理 — AI 抠图 → 去除多余透明边缘 → 按统一视觉占比放入透明或自定义颜色画布；需要单独留档透明主体时，在输出设置勾选「保留抠图结果」。
...
| **Git 客户端** | 检测本机 Git（PATH / 常见安装路径）；BEN2 等 `git+https` 依赖创建环境前需要 |
| **依赖安装镜像** | 默认清华大学 PyPI 源；创建模型环境时自动 `uv pip install -i <镜像>`，不改用户全局 pip/uv 配置，可自行修改或留空 |
| **GitHub 访问代理** | 默认 ghfast 前缀代理；安装 git 依赖与下载 uv 时改写 `github.com` 地址，不改系统 git 全局配置；可换 ghproxy / gitclone / 自定义 / 直连 |
...
| 能力 | 说明 |
|------|------|
| **格式转换** | 默认开启并输出 JPG；也可关闭或选择 PNG / WEBP / BMP / TIFF。开启后下方仅显示该格式支持的属性（BMP 无元数据字段） |
...
| **文本元素** | 三种数据源：固定文本、Excel 列数据、图片文件名；支持字体、字号、加粗、颜色、文本框宽高、自动换行及水平/垂直对齐 |
| **图片元素** | 叠加外部图片（如 Logo、水印图），支持设置位置和大小、保持比例及超出画布自动缩小 |
| **定位系统** | 支持自由 X/Y 像素坐标和动态 3×3 九宫格锚定；锚定模式会按每张底图的实际尺寸重新计算位置，可设置边距并自动限制在画布内 |
| **智能溢出处理** | 文本可在指定宽度内自动换行，空间不足时自动缩小到最小字号；图片过大时可等比缩小，避免批量处理不同尺寸底图时越界 |
...
**文本元素：**
- 定位：可选自由 X/Y 坐标或九宫格锚定；锚定后可用正负水平/垂直偏移量微调，正值向右/下、负值向左/上
- 文本框：宽度支持像素或画布百分比，高度可自动或指定；支持自动换行、水平/垂直对齐和画布边距
- 溢出：文本框越界时自动移回画布；内容过高时可逐级缩小字号到设定下限
- 内容与样式：支持固定值、Excel、文件名数据源，字体、字号、加粗、颜色可独立配置

**图片元素：**
- 定位：可选自由 X/Y 坐标或按每张底图动态计算的九宫格锚定；锚定后支持水平/垂直偏移微调和统一画布边距
- 大小：宽度/高度使用像素值；锚定模式可保持源图比例，超出底图时自动等比缩小
- 支持 PNG 透明图叠加并保留透明通道
...
- **批量处理** — 多文件添加、文件夹导入、拖放 / Ctrl+V 粘贴（含文件夹、截图、HTML/DOCX/PDF 抽图），后台线程处理不卡界面
